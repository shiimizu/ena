...
pub mod core {
    ...
    #[async_trait]
    impl QueriesExecutor<Statement, tokio_postgres::Row> for Client<tokio_postgres::Client> {
        ...
        {
            let id =
                YotsubaIdentifier { endpoint, board, statement: YotsubaStatement::UpdateMetadata };
        ...
        {
            let id =
                YotsubaIdentifier { endpoint, board, statement: YotsubaStatement::UpdateDeleteds };
            let statement = statements.get(&id).unwrap();
            Ok(self
                .execute(statement, &[
                    &serde_json::from_slice::<serde_json::Value>(item)?,
        ...
        {
            let id =
                YotsubaIdentifier { endpoint, board, statement: YotsubaStatement::UpdateThread };
            let statement = statements.get(&id).unwrap();
            Ok(self
